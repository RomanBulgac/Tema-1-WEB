    ...
    if (isset($_SESSION['login']) && isset($_SESSION['pass'])) {
        ...
        if (isset($_GET['id'])) {
            ...
            if (article::exists($conn, $id)) {
                ...
                $usersPage = new UserReg($article->author, $conn);
            } else {
        ...
        if (isset($_POST["submit"])) {
            ...
            if ($err == 0) {
                $sql = "UPDATE article SET title='{$title}', subtitle='{$subtitle}', category='{$category}', image='{$target}' WHERE id='{$id}'";
                mysqli_query($conn, $sql);
                article::writeFile($text, $id);
    ...
    <div class="row gx-4 gx-lg-5 justify-content-center">
        <div class="col-md-10 col-lg-8 col-xl-7">
            <form method="post" action="edit.php?id=<?php echo $id; ?>" enctype="multipart/form-data">
                ...
                <div><input placeholder="Upload the article cover..." data-sb-validations="required" class="form-control" type="file" name="image" id="image" accept="image/*"></div>
                <br>

                <label for="title">Title</label>
                <input class="form-control" id="title" type="text" name='title' placeholder="Enter the article title..." data-sb-validations="required" max="100" value="<?php echo $title; ?>" required /><br>
                <err style="color: #DC3545FF; font-size: 10pt"><?php echo $titleErr; ?></err>

                <label for="subtitle">Subtitle</label>
                <input class="form-control" id="subtitle" type="text" name='subtitle' placeholder="Enter the article title..." data-sb-validations="required" max="100" value="<?php echo $subtitle ?>" /><br>
                ...
                <textarea id="mytextarea" name="article"><?php echo article::readFile($id); ?> </textarea>

                <br>
                <err style="color: #DC3545FF; font-size: 10pt"><?php echo $articleErr; ?></err>
                <input class="btn btn-primary text-uppercase" id="submitButton" type="submit" name="submit" value="UPDATE!">
                <br><br>
            </form>
